...
void dijkstra(int id, int sx, int sy, int LB, int RB)
{
     ...
     for (int i = 0; i < n; i++)
        for (int j = LB; j <= RB; j++)
           d[id][i][j] = (int)1e9;
     u.push(mp(-inp[sx][sy], mp(sx,sy)));
     d[id][sx][sy] = inp[sx][sy];
     while (!u.empty())
     {
         pii cur = u.top().Y;
...
void solve(int LB, int RB, int ql, int qr)
{
    ...
    for (int ii = ql; ii <= qr; ii++)
    {
        ...
        if (qs[i].X.Y <= mid && qs[i].Y.Y >= mid)
        {
            ...
            for (int j = 0; j < n; j++)			
            {
                ans[i] = min(ans[i], d[j][qs[i].X.X][qs[i].X.Y] + d[j][qs[i].Y.X][qs[i].Y.Y] - inp[j][mid]);	
            }		
        }
    ...
    for (int i = 0; i < n; i++)
       for (int j = 0; j < n; j++)
          if (i != j)
          {
             add_edge(mp(i,mid), mp(j,mid), d[i][j][mid] - inp[i][mid]);		  
          }   
       ...
       for (int j = 0; j < m; j++)
       {
           for (int k = 0; k < 4; k++)
           {
               ...
               if (ni >= 0 && ni < n && nj >= 0 && nj < m)
                  add_edge(mp(i,j),mp(ni,nj), inp[ni][nj]);
           }
       }
