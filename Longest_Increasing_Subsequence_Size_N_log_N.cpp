...
//create method to here
 ...
 int ceil_search(int arr[], int low, int high, int n)
 {
 	if(n <= arr[low])
 	{
 		return low;
 	...
 	else if(n > arr[mid])
 	{
 		...
 		{
 			return ceil_search(arr, mid + 1, high, n);
 		}
 	}
 	else
 	{
 		if(mid - 1 >= low && n > arr[mid - 1])
 		{
 			return mid;
 		}
 		else
 		{
 			return ceil_search(arr, low, mid - 1, n);
 		}
	}
 }
...
void solve()
{
    ...
    int path[N] = { 0 }, pos_arr[N] = { 0 }, len = 0, j = 0;
    path[0] = arr[0];
    j++;
    FOR_A(i, 1, N)
    {
        ...
        {
                pos_arr[ceil_search(path, 0, j - 1, arr[i])] = i;
                path[ceil_search(path, 0, j - 1, arr[i])] = arr[i];
        }
    }
    printf("%d\n", len + 1);
}
...
//Longest_Increasing_Subsequence_Size_N_log_N.cpp